...
public class passwordEditServiceServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		user.setPassword(modifyUser.getPassword()); // 로그아웃시 필요없음 하지만 로그인을 유지한다고 하면 필요한 코드
		
		StringBuilder responseBuilder = new StringBuilder();
		responseBuilder.append("<script>");
		responseBuilder.append("alert('비밀번호 변경 완료\\n다시 로그인하세요.');");
		responseBuilder.append("location.href = '/ssa/Logout';");
		responseBuilder.append("</script>");
